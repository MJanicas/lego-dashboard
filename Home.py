...
col4.metric("Part of New Sets", metric_new_sets_parts)
...
with st.expander(f"New Sets of the Top {filt_n_themes} Master Themes",expanded=True): # Replace ... by the correct method. 🛑Remember that everything contained on the expander needs to be idented.
    ...
    sets_per_parent = df_filtered.groupby(['parent_theme_name']).set_name.nunique().reset_index().rename(columns={'set_name':'nbr_sets'})
    sets_per_parent_theme = sets_per_parent.sort_values('nbr_sets', ascending=False).reset_index()[:filt_n_themes]
    nbr_sets_remained = sets_per_parent.sort_values('nbr_sets', ascending=False).reset_index()[filt_n_themes:].nbr_sets.sum()
    ...
    fig_parent_theme.update_layout(yaxis_title = '',xaxis_title='Number Distinct Sets')
...
with st.expander(f"New Sets of the Top {filt_n_themes} Master Themes over the Years",expanded=True):
    ...
    sets_per_year_master_theme_top = sets_per_year_master_theme[sets_per_year_master_theme['parent_theme_name'].isin(sets_per_parent_theme.parent_theme_name.tolist())].copy().sort_values('parent_theme_name', ascending = True).reset_index()
...
with st.expander(f"Largest Lego Set per Years",expanded=True): # Replace ... by the code
    ...
    fig_sets_per_year = px.bar(largest_set_year, 
            x="year", 
            y="num_parts", 
            color='parent_theme_name',
            title=f'Largest Set of Every Year',
            color_discrete_sequence=colors_scaled,
            hover_name = 'theme_set')
    fig_sets_per_year.update_layout(xaxis_title='Year', 
                yaxis_title='Number of New Parts per Sets')
    ...
    st.plotly_chart(fig_sets_per_year, use_container_width = True)
